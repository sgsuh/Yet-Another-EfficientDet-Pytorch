...
def invert_affine(metas: Union[float, list, tuple], preds):
    ...
    for i in range(len(preds)):
        if preds[i]['rois'].shape[0] == 0:
            ...
            preds[i]['rois'][:, [0, 2]] = preds[i]['rois'][:, [0, 2]] *scale
            preds[i]['rois'][:, [1, 3]] = (preds[i]['rois'][:, [1, 3]]) * scale
    return preds
...
def postprocess_s(anchors, regression, classification, regressBoxes, clipBoxes, threshold, iou_threshold, tl_threshold=0.2):
    transformed_anchors = regressBoxes(anchors, regression)
    transformed_anchors = clipBoxes(transformed_anchors)

    tl_class = classification[:, 0] # w b
    other_class = classification[:, 1:] # w b

    scores = torch.max(classification, dim=2, keepdim=True)[0]

    other_scores = torch.max(other_class, dim=2, keepdim=True)[0]

    tl_scores_over_thresh = (tl_class > tl_threshold)[:] # wb
    scores_over_thresh = (other_scores > threshold)[:, 0] # wb

    scores_over_thresh = scores_over_thresh + tl_scores_over_thresh # wb

    if torch.eq(scores_over_thresh.sum(),0):
        return None, None, None

    classification_per = classification[scores_over_thresh, ...].permute(1, 0) #ob
    transformed_anchors_per = transformed_anchors[scores_over_thresh, ...] #ob
    scores_per = scores[scores_over_thresh, ...] #ob
